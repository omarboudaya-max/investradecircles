...
function InfoTab({ circle, user, isDark }) {
  const members = [
    { name: 'Alpha Manufacturing Ltd', sector: 'Manufacturing', icon: Target },
    { name: 'Nexus Tech Solutions', sector: 'Tech & IT', icon: Globe },
    { name: 'Vertex Consulting Group', sector: 'Consulting & Accounting', icon: Briefcase },
    { name: 'Global Logistics Network', sector: 'Logistics', icon: BarChart2 }
  ];

  const tenders = [
    { title: 'RFP: Smart City Infrastructure', date: 'Closes Oct 15', val: '2.5M TND' },
    { title: 'Tender: Port Expansion Materials', date: 'Closes Nov 01', val: '500K TND' }
  ];

  const events = [
    { title: 'Regional Trade Summit 2026', date: 'Next Tuesday, 10:00 AM', type: 'Seminar' },
    { title: 'B2B Matchmaking Mixer', date: 'Friday, 6:00 PM', type: 'Networking' }
  ];
  ...
  return (
    <div className="p-5 space-y-4 overflow-y-auto max-h-[70vh]">
      ...
      <div className="flex items-center gap-2 mb-1 flex-wrap">
        ...
        <span className={`text-sm font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{circle?.name}</span>
        {circle?.website_url && (
          <a
      ...
      <div className="rounded-xl p-4 border transition-all duration-300" 
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className={`text-xs font-bold uppercase tracking-wider ${isDark ? 'text-blue-300' : 'text-blue-800'}`}>Regional Mandate</span>
        </div>
        <p className={`${isDark ? 'text-white/85' : 'text-stone-800'} text-sm leading-relaxed`} dir="auto">
          {circle?.description || 'Our mission is to foster local economic growth, support our corporate network, and drive regional development through strategic trade and collaboration.'}
        </p>
      </div>
      ...
      <div className="rounded-xl p-4 border transition-all duration-300" 
        ...
        <div className="flex items-center gap-2 mb-3">
          <Users className={`w-3.5 h-3.5 ${isDark ? 'text-emerald-400' : 'text-emerald-700'}`} />
          <span className={`text-xs font-bold uppercase tracking-wider ${isDark ? 'text-emerald-300' : 'text-emerald-800'}`}>Member Directory</span>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {members.map((m, i) => (
            <div key={i} className={`p-3 rounded-lg border flex items-start gap-3 cursor-pointer hover:bg-black/5 transition-colors ${isDark ? 'border-white/10' : 'border-black/5'}`}>
      ...
      <div className="rounded-xl p-4 border transition-all duration-300" 
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className={`text-xs font-bold uppercase tracking-wider ${isDark ? 'text-amber-300' : 'text-amber-800'}`}>Economic Initiatives</span>
        </div>
        <ul className="space-y-2">
          <li className={`flex items-start gap-2 text-sm ${isDark ? 'text-white/80' : 'text-stone-800'}`}>
            <span className={`${isDark ? 'text-amber-400' : 'text-amber-600'} mt-0.5 shrink-0`}>•</span>
            Sustainability Drive 2026: Subsidizing green tech for local manufacturing.
          </li>
          <li className={`flex items-start gap-2 text-sm ${isDark ? 'text-white/80' : 'text-stone-800'}`}>
            <span className={`${isDark ? 'text-amber-400' : 'text-amber-600'} mt-0.5 shrink-0`}>•</span>
            Digital Transformation Program: E-commerce training for SMEs.
          </li>
        </ul>
      </div>

      {/* B2B Matchmaking & Tenders */}
      <div className="rounded-xl p-4 border transition-all duration-300" 
           style={{ 
             background: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(255,255,255,0.5)', 
             borderColor: isDark ? 'rgba(100,180,255,0.12)' : 'rgba(180,120,20,0.25)' 
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className={`text-xs font-bold uppercase tracking-wider ${isDark ? 'text-sky-300' : 'text-stone-800'}`}>B2B Matchmaking & Tenders</span>
        </div>
        <div className="space-y-2">
          {tenders.map((t, i) => (
            <div key={i} className="flex justify-between items-center p-2.5 rounded-lg border transition-colors" style={{ borderColor: isDark ? 'rgba(100,180,255,0.08)' : 'rgba(180,120,20,0.15)' }}>
              <div>
                <p className={`text-[12px] font-semibold ${isDark ? 'text-white' : 'text-stone-950'}`}>{t.title}</p>
                <p className={`text-[10px] mt-0.5 text-muted-foreground`}>{t.date}</p>
              </div>
              <span className="text-xs font-bold text-primary">{t.val}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-xl p-4 border transition-all duration-300" 
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className={`text-xs font-bold uppercase tracking-wider ${isDark ? 'text-blue-300' : 'text-stone-800'}`}>Upcoming Events</span>
        </div>
        <div className="space-y-2">
          {events.map((e, i) => (
            <div key={i} className="flex items-center gap-3 p-2.5 rounded-lg border transition-colors" style={{ borderColor: isDark ? 'rgba(100,180,255,0.08)' : 'rgba(180,120,20,0.15)' }}>
              <div className="flex flex-col items-center justify-center p-2 bg-primary/10 rounded-md shrink-0 w-12 text-primary text-xs font-bold">
                {e.type.charAt(0)}
              </div>
              <div>
                <p className={`text-[12px] font-semibold ${isDark ? 'text-white' : 'text-stone-950'}`}>{e.title}</p>
                <p className={`text-[10px] mt-0.5 text-muted-foreground`}>{e.date} • {e.type}</p>
              </div>
            </div>
          ))}
        </div>
...
export default function ChamberOfCommerceLayout({
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {activeTab === 'opportunities' && (
            <div className="p-0 border-t border-white/5 min-h-[400px]">
              {/* Later we can create a dedicated B2B feed component, for now re-use CircleFeed */}
